  ...
  describe 'POST #create' do
    ...
    context 'when create with invalid user' do
      shared_examples 'return error' do |description, invalid_user_params|
        ...
        it "return error status #{description}" do
          expect(response).to have_http_status(422)
        end
        it "does not create a resource #{description}" do
          expect { response }.to_not change(User, :count)
        end
        it "render the `new` template  #{description}" do
          expect(response).to render_template('new')
        end
      end
      ...
      shared_examples 'return error' do |description, params|
        ...
        params.each do |param, value|
          it "does not update user #{description}" do
            expect(user.send(param)).not_to eq(value)
          end
        end
        it "return error status #{description}" do
          expect(response).to have_http_status(303)
        end
        it "render the `edit` template #{description}" do
